        ...
        try:
            ...
            try:
                ...
                raise ConnectionError(
                    f"{self._gateway.log_id} Command response timed out after 3.0s for message `{message}`"
                ) from err
            ...
            if (
                ...
                try:
                    ...
                    raise ConnectionError(
                        f"{self._gateway.log_id} Command retry response timed out after 3.0s for message `{message}`"
                    ) from err
                resulting_message = OWNSignaling(raw_response.decode())
                if resulting_message.is_nack():
                    self._logger.error(
                ...
                try:
                    ...
                    raise ConnectionError(
                        f"{self._gateway.log_id} Follow-up response timed out after 3.0s for message `{message}`"
                    ) from err
                resulting_message = OWNSignaling(raw_response.decode())
                if resulting_message.is_nack():
                    self._logger.error(
