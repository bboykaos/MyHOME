    ...
    def parse(cls, data):
        ...
        if _match:
            ...
            if _who == 0:
                ...
                return OWNFilodiffusioneEvent(data)
            elif _who == 25:
...
class OWNCENEvent(OWNEvent):
    def __init__(self, data):
        ...
        # Standard CEN (*15*WHAT*WHERE#BUTTON##):
        # self._what is the action (0=start long, 1=short, 2=end long, 3=held)
        # self._where is the object address
        # self._where_param[0] is the pushbutton number
        if self._where_param and len(self._where_param) >= 1 and self._where_param[0].isdigit():
            self.push_button = int(self._where_param[0])
            self.object = self._where
            self._state = self._what
        elif self._what_param and len(self._what_param) >= 1:
            # Fallback for legacy format *15*BUTTON#STATE*WHERE##
            self.push_button = int(self._what)
            self.object = self._where
            try:
                self._state = int(self._what_param[0])
            except (ValueError, TypeError):
                self._state = None
        else:
            self.push_button = int(self._where_param[0]) if (self._where_param and self._where_param[0].isdigit()) else int(self._what)
            self.object = self._where
            self._state = self._what

        if self._state == 1 or self._state is None:
            self._human_readable_log = f"Button {self.push_button} of CEN object {self.object}{self._interface_log_text} short pressed."
        elif self._state in (0, 3):
            self._human_readable_log = f"Button {self.push_button} of CEN object {self.object}{self._interface_log_text} is being held pressed."
        elif self._state == 2:
            self._human_readable_log = f"Button {self.push_button} of CEN object {self.object}{self._interface_log_text} has been released after a long press."

    @property
    def is_pressed(self):
        # WHAT = 1: Short pressure
        return self._state == 1 or self._state is None

    @property
    def is_held(self):
        # WHAT = 0 (start of long pressure) or 3 (still held)
...
class OWNFilodiffusioneEvent(OWNEvent):
    """Event for BTicino Sound Diffusion (WHO 22, F500N / H4562)."""
    ...
    def __init__(self, data):
        ...
        if self._state is not None:
            self._human_readable_log = f"Filodiffusione Zone {self._zone} state: {self._state}."
        elif self._dimension is not None:
            self._human_readable_log = f"Filodiffusione Zone {self._zone} dimension: {self._dimension}."

    @property
    def is_on(self):
        return self._state == 1 or (self._state is not None and str(self._state).startswith("1"))

    @property
    def is_off(self):
        return self._state == 0 or (self._state is not None and str(self._state).startswith("0"))
...
class OWNDoorEntryEvent(OWNEvent):
    """Event for BTicino Door Entry & Locks (WHO 6, 346200 / Posto Esterno)."""

    def __init__(self, data):
        super().__init__(data)
        self._action = str(self._what) if self._what is not None else None
        self._panel = str(self._where) if self._where is not None else None

        if self._action == "10":
            self._human_readable_log = f"Door lock unlocked for entrance panel {self._panel}."
        elif self._action == "11":
            self._human_readable_log = f"Staircase light switched on by entrance panel {self._panel}."
        elif self._action == "20":
            self._human_readable_log = f"Incoming door entry call from entrance panel {self._panel}."
        else:
            self._human_readable_log = f"Door entry event action {self._action} for panel {self._panel}."
    ...
    def parse(cls, data):
        ...
        if _match:
            ...
            if _who == 0:
                ...
                return cls(data)
            elif _who == 6:  # Door Entry System (Citofonia / Serrature)
                return OWNDoorEntryCommand(data)
            elif _who == 7:
                return cls(data)
            elif _who == 9:
                return cls(data)
                ...
                return OWNFilodiffusioneCommand(data)
            elif _who == 24:
                return cls(data)
            elif _who == 25:
                _where = re.match(r"^\*.+\*(?P<where>\d+)##$", data).group("where")
...
class OWNAutomationCommand(OWNCommand):
    ...
    def set_shutter_level(cls, where, level=30):
        # Dimension 10: Absolute Position (0 = closed, 100 = fully open)
        message = cls(f"*#2*{where}*#10*{level}##")
        message._human_readable_log = f"Setting shutter {message._where}{message._interface_log_text} position to {level}%."
        return message

    @classmethod
    def set_slat_angle(cls, where, angle=0):
        # Dimension 11: Venetian blind slat angle (0-100 degrees)
        message = cls(f"*#2*{where}*#11#001*{angle}##")
        message._human_readable_log = f"Setting shutter {message._where}{message._interface_log_text} slat angle to {angle}°."
        return message
...
class OWNSoundCommand(OWNCommand):
    ...
    def turn_on(cls, where):
        # WHAT = 1: Turn ON amplifier (last source)
        message = cls(f"*16*1*{where}##")
        message._human_readable_log = f"Turning ON audio zone {where}."
        return message

    @classmethod
    def turn_off(cls, where):
        # WHAT = 0: Turn OFF amplifier in room
...
class OWNFilodiffusioneCommand(OWNCommand):
    """Commands for BTicino Sound Diffusion (WHO 22, F500N / H4562)."""

    @classmethod
    def status(cls, where):
        message = cls(f"*#22*{where}*12##")
        message._human_readable_log = f"Requesting filodiffusione zone {where} status."
        return message

    @classmethod
    def turn_on(cls, where):
        message = cls(f"*22*1#4#7*{where}##")
        message._human_readable_log = f"Turning ON filodiffusione zone {where}."
        return message

    @classmethod
    def turn_off(cls, where):
        # Fast Turn-off: audio source disconnection
        message = cls(f"*22*1#4#0*{where}##")
        message._human_readable_log = f"Turning OFF filodiffusione zone {where}."
        return message

    @classmethod
    def turn_off_standby(cls, where):
        # Amplifier hardware standby
        message = cls(f"*22*0#4#0*{where}##")
        message._human_readable_log = f"Standby filodiffusione zone {where}."
        return message

    @classmethod
    def set_volume(cls, where, volume):
        message = cls(f"*#22*{where}*#1*{volume}##")
        message._human_readable_log = f"Setting volume to {volume} for filodiffusione zone {where}."
        return message

    @classmethod
    def volume_up(cls, where):
        message = cls(f"*22*3#1*{where}##")
        message._human_readable_log = f"Turning UP volume for filodiffusione zone {where}."
        return message

    @classmethod
    def volume_down(cls, where):
        message = cls(f"*22*4#1*{where}##")
        message._human_readable_log = f"Turning DOWN volume for filodiffusione zone {where}."
        return message


class OWNDoorEntryCommand(OWNCommand):
    """Command for BTicino Door Entry & Locks (WHO 6, 346200 / Posto Esterno)."""

    @classmethod
    def unlock(cls, where: str = "0"):
        """Unlock door / trigger electric lock strike for entrance panel or relay where."""
        message = cls(f"*6*10*{where}##")
        message._human_readable_log = f"Unlocking door lock for entrance panel / relay {where}."
        return message

    @classmethod
    def staircase_light(cls, where: str = "0"):
        """Turn on staircase light via door entry subsystem."""
        message = cls(f"*6*11*{where}##")
        message._human_readable_log = f"Turning on staircase light via door entry {where}."
        return message
